...
# Extracting the visits during the A/B testing period only from the full dataset
df = sql_query('''
...
#importing both pandas and matplotlib for testing of data
import pandas as pd
from matplotlib import pyplot as plt
...
#Adding in ab test group column to split data into the relevant testing pools
...
#Grouping by the testing group for analysis and plotting
...
#Plotting the testing groups in a pie to show the almost 50/50 split
...
#Adding column to include a split of those who made an application
...
# Grouping by A/B group and of those groups how many made an application
...
#Run a pivot to make the data more obvious in review
...
#Add a total column for applications + no application
...
# Run a percentage of Applications from the total to understand the uptake
...
""" Run a Chi Square Test to assertain if the results above show that one group was more
likely than the other to pick up an application
"""
...
#Adding column to include a split of those who became a member or not
df['is_member'] = df.purchase_date.apply(lambda x:
                                                'Member' if pd.notnull(x) else 'Non Member')
...
#defining a dataframe for those who picked up an application and are now members
...
#Run a pivot off the members with applications dataframe
...
member_pivot['Total'] = member_pivot.Member + member_pivot['Non Member']
...
""" Run a Chi Square Test to assertain if the results above show that one group was more
likely than the other to become members after picking up an application
"""
from scipy.stats import chi2_contingency
...
# Final dataframe that shows the percentages of Members to Non Members for the overall total visitors
...
final_member_pivot['Total'] = final_member_pivot.Member + final_member_pivot['Non Member']
...
""" Run a Chi Square Test to assertain if the results above show that one group was more
likely than the other to become a member 
"""
from scipy.stats import chi2_contingency
...
# Percentage of Visitors who actually apply
ax = plt.subplot()
